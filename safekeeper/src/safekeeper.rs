...
use crate::state::SafekeeperState;
...
pub struct SafeKeeper<CTRL: control_file::Storage, WAL: wal_storage::Storage> {
    ...
    pub state: SafekeeperState<CTRL>, // persistent state storage
    pub wal_store: WAL,
    ...
    WAL: wal_storage::Storage,
{
    ...
    pub fn new(state: CTRL, wal_store: WAL, node_id: NodeId) -> Result<SafeKeeper<CTRL, WAL>> {
        ...
        Ok(SafeKeeper {
            ...
            state: SafekeeperState::new(state),
            wal_store,
            node_id,
        })
    }
    ...
    async fn handle_elected(
        ...
        // and now adopt term history from proposer
        {
            ...
            // upgrade.
            state.commit_lsn = max(self.state.inmem.commit_lsn, state.timeline_start_lsn);

            // Initializing backup_lsn is useful to avoid making backup think it should upload 0 segment.
            state.backup_lsn = max(self.state.inmem.backup_lsn, state.timeline_start_lsn);
...
mod tests {
    ...
    struct InMemoryState {
        persisted_state: SafeKeeperPersistentState,
    }
    ...
    fn test_sk_state() -> SafeKeeperPersistentState {
        let mut state = SafeKeeperPersistentState::empty();
    ...
    fn test_sk_state_bincode_serde_roundtrip() {
        ...
        let state = SafeKeeperPersistentState {
            tenant_id,
            timeline_id,
        ...
        let deser = SafeKeeperPersistentState::des(&ser).unwrap();
