...
pub struct SafeKeeperPersistentState {
    #[serde(with = "hex")]
...
impl SafeKeeperPersistentState {
    pub fn new(
...
pub struct SafekeeperState<CTRL: control_file::Storage> {
    pub inmem: SafekeeperMemState,
    pub pers: CTRL, // persistent
}

impl<CTRL> SafekeeperState<CTRL>
where
    CTRL: control_file::Storage,
{
    ...
    pub fn start_change(&self) -> SafeKeeperPersistentState {
    ...
    pub async fn finish_change(&mut self, s: &SafeKeeperPersistentState) -> Result<()> {
...
impl<CTRL> Deref for SafekeeperState<CTRL>
where
    CTRL: control_file::Storage,
{
