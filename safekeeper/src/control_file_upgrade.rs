...
pub fn upgrade_control_file(buf: &[u8], version: u32) -> Result<SafeKeeperPersistentState> {
    ...
    if version == 1 {
        ...
        return Ok(SafeKeeperPersistentState {
    ...
    // migrate to having timeline_start_lsn
        ...
        info!("reading safekeeper control file version {}", version);
        let mut oldstate = SafeKeeperPersistentState::des(&buf[..buf.len()])?;
