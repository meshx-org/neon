...
Shard  | 00 | 01 | 02 | 00 | 01 | 02 | 00 | 01 | 02 | 00 | ...
```
...
The shard number will be included as a prefix, like this:

`pageserver/v1/tenants/<tenant_id>/timelines/<timeline id>/<shard_number><shard_count>/<layer file name>-<generation>`

`pageserver/v1/tenants/<tenant_id>/timelines/<timeline id>/<shard_number><shard_count>/index_part.json-<generation>`
...
- The prefix comes _after_ the `/timelines/` part, so that one may still list the timelines
  in a tenant without having to walk the shards.
